...
export default function BreathGuide({
  ...
  return (
    <div className="flex flex-col items-center gap-3">
      ...
      >
        <div
          className={`${circleSize} rounded-full bg-sage transition-all duration-1000 ease-in-out dark:bg-sage/80`}
          style={{
      ...
      <span className={`${textSize} font-light tracking-wide text-charcoal/50 dark:text-cream/50`}>
        {PHASE_LABELS[phase]}
        {active && cycleCount > 0 && phase === "inhale" && countdown === PHASE_DURATIONS.inhale && (
          <span className="ml-2 text-charcoal/30 dark:text-cream/30">· cycle {cycleCount + 1}</span>
        )}
      </span>
    </div>
  );
